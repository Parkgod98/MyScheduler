...
`우리은행 9월 8일 마감`처럼 자연어로 입력하면 날짜·시간·분류·메모를 해석해 일정으로 등록하고, PC와 휴대폰에서 같은 내용을 확인할 수 있습니다.
...
처음에는 LLM으로 문장을 해석하는 방법도 생각했지만, 일정 입력은 표현 범위가 비교적 명확하고 결과가 매번 같아야 했습니다. 그래서 별도 API 비용 없이 동작하는 규칙 기반 한국어 파서를 구현했습니다.

덕분에 같은 입력은 항상 같은 결과로 해석되고, 사용자 일정이 외부 LLM으로 전송되지 않습니다.
