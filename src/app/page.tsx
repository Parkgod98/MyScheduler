...
const STORAGE_KEY = "myscheduler.events.v3";
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="topbar-inner">
          <div>
            ...
            <div className="top-summary">{nextEvent ? `${dday(nextEvent.startsAt, now)} · ${nextEvent.title}` : "예정된 일정이 없습니다"}</div>
          </div>
          <button className="icon-button" onClick={enableNotifications} aria-label="알림 켜기"><Bell size={20} /></button>
        </div>
      </header>
      ...
      <main className="main-content">
        {view === "calendar" && (
          <>
            <section className="overview-row">
              <div><span>이번 주</span><strong>{dueSoon}개</strong></div>
              <div><span>다가오는 일정</span><strong>{upcomingTasks.length}개</strong></div>
              <div className="overview-next"><span>다음 일정</span><strong>{nextEvent ? nextEvent.title : "없음"}</strong></div>
            </section>

            <section className="calendar-toolbar">
              <div className="month-title">{format(month, "yyyy년 M월", { locale: ko })}</div>
              <div className="month-actions">
                <button onClick={() => setMonth(subMonths(month, 1))}><ChevronLeft size={18} /></button>
                <button onClick={() => { const today = new Date(); setMonth(today); setSelected(today); }}>오늘</button>
                <button onClick={() => setMonth(addMonths(month, 1))}><ChevronRight size={18} /></button>
              </div>
            </section>

            <div className="legend">
              {(Object.entries(categoryLabels) as [EventCategory, string][]).map(([key, label]) => <span key={key}><i className={`dot dot-${key}`} />{label}</span>)}
            </div>
            ...
            <section className="calendar-card">
              <div className="calendar-grid">
                ...
                {days.map((day) => {
                  const dayEvents = sortEvents(events.filter((item) => isSameDay(new Date(item.startsAt), day)));
                  return (
                    <button key={day.toISOString()} className={`day ${!isSameMonth(day, month) ? "muted" : ""} ${isSameDay(day, new Date(now)) ? "today" : ""}`} onClick={() => openDate(day)}>
                      <span className="day-number">{format(day, "d")}</span>
                      <div className="day-events">
                        {dayEvents.slice(0, 2).map((item) => (
                          <div key={item.id} className={`calendar-event event-${item.category} ${item.completed ? "done" : ""}`}>
                            <span className="calendar-event-title">{item.title}</span>
                            <span className="calendar-event-time">{format(new Date(item.startsAt), "HH:mm")}</span>
                          </div>
                        ))}
        ...
        {view === "tasks" && (
          <section className="tasks-page">
            <div className="page-heading"><div><span className="eyebrow">TASKS</span><h1>다가오는 일정</h1></div><span>{filteredTasks.length}개</span></div>
            <div className="filter-strip">
              <button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>전체</button>
              {(Object.entries(categoryLabels) as [EventCategory, string][]).map(([key, label]) => <button className={filter === key ? "active" : ""} key={key} onClick={() => setFilter(key)}>{label}</button>)}
            </div>
            <div className="task-list">
              {filteredTasks.length === 0 && <div className="empty-state">조건에 맞는 일정이 없습니다.</div>}
              {filteredTasks.map((item) => <TaskCard key={item.id} item={item} now={now} onPatch={patchEvent} onDelete={removeEvent} />)}
            </div>
          </section>
        )}
        ...
        {view === "add" && (
          <section className="add-page">
            <div className="page-heading"><div><span className="eyebrow">QUICK ADD</span><h1>말하듯 적어주세요</h1></div><Sparkles size={24} /></div>
            <p className="page-description">한 줄에 하나씩 적으면 날짜·시간·종류·알림을 자동으로 정리합니다.</p>
            <textarea className="natural-input" value={naturalText} onChange={(event) => setNaturalText(event.target.value)} rows={7} placeholder={"현대자동차 9월 14일 마감\nKBS 필기 발표 9월 14일\nKBS 시험 9월 30일 오전 9시 메모: 여권 챙기기"} />
            <button className="primary wide" onClick={makePreview}><Sparkles size={17} /> 일정 해석하기</button>
            {preview.length > 0 && <div className="preview-list">
              {preview.map((item, index) => <div className="preview-card" key={`${item.startsAt}-${index}`}>
                <div className="preview-head"><span className={`category-badge badge-${item.category}`}>{categoryLabels[item.category]}</span><button onClick={() => setPreview((current) => current.filter((_, i) => i !== index))}><X size={16} /></button></div>
                ...
                <div className="preview-row">
                  <select value={item.category} onChange={(event) => updatePreview(index, { category: event.target.value as EventCategory })}>{(Object.entries(categoryLabels) as [EventCategory, string][]).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
                ...
                <textarea value={item.notes} onChange={(event) => updatePreview(index, { notes: event.target.value })} rows={2} placeholder="메모 / 준비물" />
              </div>)}
              <button className="primary wide" onClick={() => void persistSchedules(preview)}>{preview.length}개 일정 등록</button>
            </div>}
          </section>
        )}
        ...
        {view === "settings" && (
          <section className="settings-page">
            <div className="page-heading"><div><span className="eyebrow">SETTINGS</span><h1>설정</h1></div></div>
            <div className="settings-card">
              <h2>계정</h2>
              {supabase ? userId ? <><p>Supabase 동기화가 연결되어 있습니다.</p><button className="secondary" onClick={() => supabase.auth.signOut()}>로그아웃</button></> : <div className="login-form"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="이메일" /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="비밀번호" /><button className="primary" onClick={signIn}>로그인</button><button className="secondary" onClick={signUp}>회원가입</button></div> : <p>현재 로컬 모드입니다.</p>}
            </div>
            <div className="settings-card"><h2>알림</h2><p>이 기기에서 백그라운드 Push 알림을 받습니다.</p><button className="secondary" onClick={enableNotifications}><Bell size={16} /> 알림 켜기</button></div>
            <div className="settings-card"><h2>Android 홈 위젯</h2><p>네이티브 Android 위젯은 다음 개발 단계에서 추가합니다. 작은 위젯은 다음 일정, 큰 위젯은 다가오는 3~5개 일정을 표시할 예정입니다.</p></div>
          </section>
        )}
      ...
      <nav className="bottom-nav" aria-label="주요 메뉴">
        <button className={view === "calendar" ? "active" : ""} onClick={() => setView("calendar")}><CalendarDays size={21} /><span>캘린더</span></button>
...
function DateSheet({ date, items, now, onClose, onPatch, onDelete, onAdd }: { date: Date; items: ScheduleEvent[]; now: number; onClose: () => void; onPatch: (item: ScheduleEvent, patch: Partial<Pick<ScheduleEvent, "notes" | "checklist" | "completed">>) => Promise<void>; onDelete: (id: string) => Promise<void>; onAdd: () => void; }) {
  return <div className="sheet-backdrop" onMouseDown={onClose} role="presentation"><section className="bottom-sheet" onMouseDown={(event) => event.stopPropagation()} aria-modal="true" role="dialog"><div className="sheet-handle" /><div className="sheet-header"><div><span className="eyebrow">{format(date, "yyyy.MM.dd")}</span><h2>{format(date, "M월 d일 EEEE", { locale: ko })}</h2></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div><div className="sheet-list">{items.length === 0 ? <div className="empty-state">등록된 일정이 없습니다.</div> : items.map((item) => <TaskCard key={item.id} item={item} now={now} onPatch={onPatch} onDelete={onDelete} />)}</div><button className="primary wide" onClick={onAdd}><Plus size={17} /> 이 날짜에 일정 추가</button></section></div>;
}
...
function TaskCard({ item, now, onPatch, onDelete }: { item: ScheduleEvent; now: number; onPatch: (item: ScheduleEvent, patch: Partial<Pick<ScheduleEvent, "notes" | "checklist" | "completed">>) => Promise<void>; onDelete: (id: string) => Promise<void>; }) {
  ...
  return <article className={`task-card ${item.completed ? "completed" : ""}`}>
    ...
    <div className="task-actions"><button className="secondary" onClick={() => void onPatch(item, { notes, checklist })}>메모·준비물 저장</button><button className="danger-link" onClick={() => void onDelete(item.id)}>삭제</button></div>
  </article>;
}
